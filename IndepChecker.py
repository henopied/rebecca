...
class IndepChecker(object):
    ...
    def __process_circuit(self):
        for node in self.__circuit.nodes():
            ...
            if node_type == "port":
                ...
                logger.error(
                    "Unknown node type {} for the node {}".format(node_type, node)
                )
                exit(-1)
    ...
    def __process_linear_gate(self, gate):
        ...
        if len(pred) == 2:
            ...
            if self.__mode == "transient":
                self.__s.add(
                    Or(
                        self.__z3_empty(self.__variables_transient[gate]),
                        self.__z3_copy(
                            self.__variables_transient[in1],
                            self.__variables_transient[gate],
                        ),
                        self.__z3_copy(
                            self.__variables_transient[in2],
                            self.__variables_transient[gate],
                        ),
                        self.__z3_xor(
                            self.__variables_transient[in1],
            ...
            self.__s.add(
                self.__z3_copy(
                    self.__variables_stable[in1], self.__variables_stable[gate]
                )
            )
            if self.__mode == "transient":
                self.__s.add(
            ...
            logger.error(
                "Number of inputs for the gate {} should be equal to one or two".format(
                    gate
                )
            )
            exit(-1)
    ...
    def __process_nonlinear_gate(self, gate):
        ...
        if len(pred) == 2:
            ...
            self.__s.add(
                Or(
                    self.__z3_empty(self.__variables_stable[gate]),
                    self.__z3_copy(
                        self.__variables_stable[in1], self.__variables_stable[gate]
                    ),
                    self.__z3_copy(
                        self.__variables_stable[in2], self.__variables_stable[gate]
                    ),
                    self.__z3_xor(
                        self.__variables_stable[in1],
                        self.__variables_stable[in2],
    ...
    def __process_not_gate(self, gate):
        ...
        if len(pred) == 1:
            ...
            logger.error(
                "Number of inputs for the gate {} should be equal to one".format(gate)
            )
            exit(-1)
    ...
    def __process_register_gate(self, gate):
        ...
        if len(pred) == 1:
            ...
            if self.__mode == "transient":
                self.__s.add(
                    self.__z3_copy(
                        self.__variables_stable[in1], self.__variables_transient[gate]
                    )
                )
        else:
            logger.error(
    ...
    def __checker_init(self):
            ...
            if self.__circuit[node]["node_type"] == "port":
                for label in self.__labels[str(node)]:
                    ...
                    if label_type == "s":
                        ...
                        logger.error(
                            "Unknown label type of the label {} for the node {}".format(
                                label, node
                            )
                        )
                        exit(-1)
        ...
        for node in self.__circuit.nodes():
            self.__variables_stable[node] = [
                Bool("{}_{}_stable".format(v, node)) for v in variables
            ]
            variables_activation[node] = Bool("activation_{}".format(node))
            if self.__mode == "transient":
                self.__variables_transient[node] = [
                    Bool("{}_{}_transient".format(v, node)) for v in variables
                ]
        ...
        for share in self.__shares:
            indep_activation_sums[share] = Sum(
                [If(shares_acivation[share][n], 1, 0) for n in self.__shares[share]]
            )
            self.__s.add(indep_activation_sums[share] <= self.__order)
            self.__s.add(indep_activation_sums[share] >= 0)
        indep_activation_sums["output"] = Sum(
            [
                If(shares_acivation["output"][n], 1, 0)
                for n in shares_acivation["output"]
            ]
        )
        ...
        for var in variables:
            ...
            for share in shares_acivation:
                for node in shares_acivation[share]:
                    try:
                        ind = [
                            str(i) for i in self.__variables_transient[int(node)]
                        ].index("{}_{}_transient".format(var, node))
                        lst += [
                            And(
                                shares_acivation[share][node],
                                self.__variables_transient[int(node)][ind],
                            )
                        ]
                    except:
                        ind = [str(i) for i in self.__variables_transient[node]].index(
                            "{}_{}_transient".format(var, node)
                        )
                        lst += [
                            And(
                                shares_acivation[share][node],
                                self.__variables_transient[node][ind],
                            )
                        ]
