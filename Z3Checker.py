...
class Z3Checker(object):
    ...
    def __checker_init(self):
        ...
        if self.__check_security:
            activation_sum = Sum(
                [
                    If(variables_activation[node], 1, 0)
                    for node in self.__circuit.nodes()
                ]
            )
            ...
            for var in variables:
                ...
                for node in self.__circuit.nodes():
                    if self.__mode == "transient":
                        ...
                        lst += [
                            And(
                                variables_activation[node],
                                self.__variables_transient[node][ind],
                            )
                        ]
                    else:
                        ind = [str(i) for i in self.__variables_stable[node]].index(
                            "{}_{}_stable".format(var, node)
                        )
                        lst += [
                            And(
                                variables_activation[node],
                                self.__variables_stable[node][ind],
                            )
                        ]
