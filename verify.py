...
def verify_circuit(
    circuit_file, labeling, order, mode="transient", log="tmp/report.txt"
):
    secrets = ", ".join([var for k in labeling for var in labeling[k] if "s_" in var])
    labels = labeling
    ...
    logger.info(
        "... secrets {} are checked in {}h{}m{}s".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--optimized", action="store_true", help="run verification in parallel"
    )
    parser.add_argument(
        "-c",
        "--check",
        nargs=4,
        metavar=("<netlist>", "<order>", "<labeling>", "<mode>"),
    ...
    parser.add_argument(
        ...
        "--independence-check",
        nargs=3,
        metavar=("<netlist>", "<order>", "<labeling>"),
    ...
    if args["check"]:
        ...
        if args["check"][3] == "t":
            ...
            exit()
        logger.info(
            "Verifying {} for {} order in {} mode".format(args["check"][0], order, mode)
        )
        for l in labels:
            logger.info(
                "Initial labeling:\n{}".format(get_pretty_labeling(l, args["check"][2]))
            )
        pool_len = len(labels) if len(labels) <= 10 else 10
        with Pool(pool_len) as p:
            res = p.starmap(
                verify_circuit, [(args["check"][0], l, order, mode) for l in labels]
            )
        for r in res:
            if not r[0]:
                print(r)
