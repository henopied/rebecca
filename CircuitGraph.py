...
class CircuitGraph(object):
    ...
    def __parse_json(self, circuit_json):
        ...
        for n in circuit_json["netnames"]:
            ...
            for b in bits:
                if b not in netnames:
                    ...
                    logger.info("parse json: the netname {} already exists".format(b))

        for c in circuit_json["cells"]:
            cell_type = circuit_json["cells"][c]["type"].split("_")[1].lower()
            cell_num = c.split("$")[-1]
        ...
        for c in circuit_json["cells"]:
            ...
            name = "{}_{}".format(cell_type, cell_num)
            if (
                "port_directions" in circuit_json["cells"][c]
                and "connections" in circuit_json["cells"][c]
            ):
                ...
                for d in directions:
                    if type(connections[d][0]) != int:
                        if connections[d][0] not in netnames:
                            ...
                            logger.info(
                                "parse json: the const cell {} already exists".format(
                                    connections[d][0]
                                )
                            )
                    if directions[d] == "input":
                        if type(connections[d][0]) != int:
                            self.__add_wire("const_{}".format(connections[d][0]), name)
                        else:
                ...
                logger.warn(
                    "parse json: there is no port or connections for the cell {}".format(
                        c
                    )
                )
            ...
            if "input" in wires[w]:
                for i in wires[w]["input"]:
                    if "output" in wires[w]:
                        ...
                        self.__add_wire(i, w)
                        logger.warn(
                            "parse json: there is no output for wire {}".format(w)
                        )
            else:
                for o in wires[w]["output"]:
                    self.__add_wire(w, o)
                logger.warn("parse json: there is no input for wire {}".format(w))
    ...
    def __add_cell(self, name, ctype, label=None):
        if name not in self.__redundant_circuit["cells"]:
            ...
            logger.warn(
                "add cell: the cell {} ({}) is already exists".format(name, ctype)
            )
    ...
    def __construct_graph(self):
            ...
            if node_type == "not":
                for p in self.__redundant_graph.predecessors(n):
                    for o in self.__redundant_graph.successors(n):
                        if (
                            p in self.__circuit["cells"]
                            and o in self.__circuit["cells"]
                        ):
        ...
        for n in self.__graph.nodes():
            ...
            if t in ("and", "xor") and (s < 1 or p != 2):
                logger.warn(
                    "Construct graph: node {} is suspicious: predecessors = {}, successors = {}".format(
                        n, p, s
                    )
                )
